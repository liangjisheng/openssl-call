...
void myEncode_1()
{
	string str = "test";
	string str_encode = "";
	const char* pszstr = "tes";
	char *pszstr_encode = new char[10];
	memset(pszstr_encode, 0, 10);

	base64::encoder e;
	cout << "before base64:" << pszstr << endl;
	cout << "before length = " << strlen(pszstr) << endl;
	// 如果源字符串的字节数不是3的倍数,转换后会在目标字符串后面加一个换行符
	int nCount = e.encode(pszstr, strlen(pszstr), pszstr_encode);
	cout << "trans bytes = " << nCount << endl;
	cout << "after base64:" << pszstr_encode << endl;
	cout << "after length = " << strlen(pszstr_encode) << endl;

	delete []pszstr_encode;
	pszstr_encode = NULL;


	string strTest = "Man is distinguished, not only by his reason, but by this singular passion from other animals, which is a lust of the mind, that by a perseverance of delight in the continued and indefatigable generation of knowledge, exceeds the short vehemence of any carnal pleasure.";
	char *pRes = new char[1024];
	memset(pRes, 0, 1024);
	nCount = e.encode(strTest.c_str(), strTest.size(), pRes);
	cout << "trans bytes = " << nCount << endl;
	cout << "before base64 :" << strTest << endl;
	cout << "after base64 : " << pRes << endl;
...
int main(int argc, char** argv)
{
	...
	// std::string input = "test1.jpg";
	// std::string input = "test_1.txt";
	std::string input = "BmpDataBase64.txt";
	...
	// std::string output = "encode_test1.txt";
	std::string output = "head.bmp";
