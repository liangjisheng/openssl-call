...
char* decode(const char* input)
{
	...
	int nLen = strlen(input);
	// char* output = (char*)malloc(SIZE);
	char* output = (char*)malloc(nLen + 1);
	//memset(output, 0 , nLen + 1);
	...
	*c = 0;
	...
	// cout << output << " length is " << cnt << endl;
	printf("%s\n", output);
	printf("%d\n", cnt);
...
int main()
{
	...
	printf("encoded: %s\n", encoded); /* encoded data has a trailing newline */
