...
void TDES::key_schedule(uchar key[], uchar schedule[][6], uint mode)
{
   uint i,j,to_gen,C,D,
...
void TDES::IP(uint state[], uchar in[])
{
...
void TDES::InvIP(uint state[], uchar in[])
{
...
uint TDES::f(uint state, uchar key[])
{
   uchar lrgstate[6];
   uint t1,t2;
...
void TDES::des_crypt(uchar in[], uchar out[], uchar key[][6])
{
   uint state[2],idx,t;
...
void TDES::three_des_key_schedule(uchar key[], uchar schedule[][16][6], uint mode)
{
   if (mode == ENCRYPT) {
...
void TDES::three_des_crypt(uchar in[], uchar out[], uchar key[][16][6])
{
...
void TDES::des_3(uchar *d,uchar *e,uchar *f,uchar mode)
{
   uchar	three_schedule[3][16][6];

   three_des_key_schedule(e,three_schedule,mode);
   three_des_crypt(d,f,three_schedule);
}