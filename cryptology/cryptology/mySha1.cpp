...
bool mySHA1::SHA_GO( const char *lpData_Input,int InputLen,char *lpSHACode_Output)
{
	...
	for(int pos=0 ; pos<=InputLen ; pos+=64)
	{
		...
		{
			nDealDataLen = InputLen - pos;			//	input-data isn't enough fill 64bytes,need fill 0x8000000000 and lenth of real-data.
			memset(Message_Block, 0, sizeof(Message_Block));
			memcpy(Message_Block, lpData_Input + pos, nDealDataLen);
...
void mySHA1::AddDataLen(int nDealDataLen)
{
 ...
 *
 */
void mySHA1::ProcessMessageBlock()
{
 ...
 *
 */
unsigned mySHA1::CircularShift(int bits, unsigned word)
{
